...
func (r *MappingRepo) MigrateTable() error {
	return db.DB.AutoMigrate(&entity.MappingArticleTag{})
}

// CreateArticleTag 创建文章标签映射
func (r *MappingRepo) CreateArticleTag(articleID, tagID int) error {
	return db.DB.Create(&entity.MappingArticleTag{ArticleID: articleID, TagID: tagID}).Error
}

// DeleteArticleTag 删除文章标签映射 通过文章ID和标签ID
func (r *MappingRepo) DeleteArticleTag(articleID, tagID int) error {
	return db.DB.Where("article_id = ? and tag_id = ?", articleID, tagID).Delete(&entity.MappingArticleTag{}).Error
}
...
func (r *MappingRepo) DeleteArticleTagByTagIds(articleID int, tagIds []int) error {
	return db.DB.Where("article_id = ? and tag_id in ?", articleID, tagIds).Delete(&entity.MappingArticleTag{}).Error
}

// DeleteArticle 删除文章
func (r *MappingRepo) DeleteArticle(articleID int) error {
	return db.DB.Where("article_id = ?", articleID).Delete(&entity.MappingArticleTag{}).Error
}

// DeleteTag 删除标签
func (r *MappingRepo) DeleteTag(tagID int) error {
	return db.DB.Where("tag_id = ?", tagID).Delete(&entity.MappingArticleTag{}).Error
}
...
func (r *MappingRepo) ListArticleIdsByTagIds(ctx *context.Context, tagIds []int) (articleIds []int, err error) {
	err = db.DB.Model(&entity.MappingArticleTag{}).Scopes(gormx.Context(ctx)).Where("tag_id in ?", tagIds).Pluck("article_id", &articleIds).Error
	return
}
...
func (r *MappingRepo) ListTagIdByArticleIds(ctx *context.Context, articleIds []int) (tagIds []int, err error) {
	err = db.DB.Model(&entity.MappingArticleTag{}).Scopes(gormx.Context(ctx)).Where("article_id in ?", articleIds).Pluck("tag_id", &tagIds).Error
	return
}

// CountByTagID 根据tagID统计
func (r *MappingRepo) CountByTagID(tagID int) (res int64, err error) {
	err = db.DB.Model(entity.MappingArticleTag{}).Where("tag_id = ?", tagID).Count(&res).Error
	return
}
